...
# as the script writing to S3 is already multithreaded we decided not to use this any more @echo "- tmscreatequeue     Creates AWS SQS queue with given settings (all tiles)"
# as the script writing to S3 is already multithreaded we decided not to use this any more @echo "- tmsdeletequeue     Deletes current AWS SQS queue (you loose everything)"
# as the script writing to S3 is already multithreaded we decided not to use this any more @echo "- tmsqueuestats      Get stats of AWS SQS queue"
# as the script writing to S3 is already multithreaded we decided not to use this any more @echo "- tmscreatetiles     Creates tiles using the AWS SQS queue"
# as the script writing to S3 is already multithreaded we decided not to use this any more @echo "- tilejson           Creates a tilejson provided a given template (usage: make tilejson TILEJSON_TEMPLATE=..."
